...
function loadForgeArtifact(contractName) {
    const artifactPath = path.join(__dirname, `../dist/contracts/${contractName}.sol/${contractName}.json`);
...
async function main() {
    ...
    // Load SimpleEscrowFactory artifact
    const escrowFactoryArtifact = loadForgeArtifact("SimpleEscrowFactory");
    
    // Deploy SimpleEscrowFactory
    console.log("1️⃣  Deploying SimpleEscrowFactory...");
    
    const SimpleEscrowFactory = new ethers.ContractFactory(
    ...
    const escrowArtifact = loadForgeArtifact("SimpleEscrow");
