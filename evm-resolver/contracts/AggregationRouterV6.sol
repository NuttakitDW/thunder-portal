  ...
  PermitAndCall
{
  ...
  function _fill(
    ...
    if (order.makerTraits.useBitInvalidator()) {
      ...
      _remainingInvalidator[order.maker.get()][orderHash] = RemainingInvalidatorLib.remains(
        remainingMakingAmount,
        makingAmount
      );
    }
  ...
  function _parseArgs(
    TakerTraits takerTraits,
    bytes calldata args
  ) private view returns (address target, bytes calldata extension, bytes calldata interaction) {
  ...
  function swap(
    ...
    if (desc.flags & _PARTIAL_FILL != 0) {
      ...
      if (returnAmount * desc.amount < desc.minReturnAmount * spentAmount)
        revert RouterErrors.ReturnAmountIsNotEnough(returnAmount, (desc.minReturnAmount * spentAmount) / desc.amount);
    } else {
...
contract UnoswapRouter is Pausable, EthReceiver, IUniswapV3SwapCallback {
  ...
  uint256 private constant _SELECTORS = ((uint256(uint32(IUniswapV3Pool.token0.selector)) << 224) |
  ...
  function _unoswapV2(address recipient, uint256 amount, uint256 minReturn, Address dex) private returns (uint256 ret) {
      ...
      if iszero(and(shr(_CURVE_SWAP_HAS_ARG_DESTINATION_OFFSET, dex), 0x01)) {
        ...
        if xor(recipient, address()) {
          ...
          default {
            ...
            if iszero(
              _asmCall(toToken, 0xa9059cbb00000000000000000000000000000000000000000000000000000000, recipient, ret, ptr)
            ) {
